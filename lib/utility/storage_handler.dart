...
class AppDataStorageManager {
  ...
  Future<String> readFromFile(String url) async {
    ...
    if (isFileExist) {
      ...
      print("1) I AM STORAGE, NOW FILE CONTAINS : $fileContents CHECKING AT THE TIME OF THIS URL : $url"); //DEBUG
      if (fileContents.contains(url)) {
        print("2) I AM STORAGE, AND I FOUND URL IN FILE SO IT IS UNSAFE : $url");
        return "UNSAFE";
      } else {
        print("2) I AM STORAGE, URL NOT FOUND IN FILE SO REQUESTING TO API : $url");
        final apiResponse = await makeRequest(url);

        if (apiResponse == "MALWARE" || apiResponse == "SOCIAL_ENGINEERING") {
          print("3) I AM STORAGE, AND I GOT URL IS UNSAFE RESULT FROM API : $url");
          return "UNSAFE";
        } else {
          print("3) I AM STORAGE, AND I GOT URL IS SAFE RESULT FROM API : $url");
          return "SAFE";
        }
      }
    } else {
